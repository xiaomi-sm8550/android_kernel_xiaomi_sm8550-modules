...
static int mi_disp_procfs_pps_rw_open(struct inode *inode, struct file *file)
{
	return single_open(file, mi_disp_procfs_pps_rw_show, PDE_DATA(inode));
}
...
static int mi_disp_procfs_dsi_phy_rw_init(void *d_display, int disp_id)
{
	struct disp_display *dd_ptr = (struct disp_display *)d_display;
	struct disp_core *disp_core = mi_get_disp_core();
	int ret = 0;
...
static int mi_disp_procfs_dsi_phy_rw_deinit(void *d_display, int disp_id)
{
	struct disp_display *dd_ptr = (struct disp_display *)d_display;
	int ret = 0;
	...
	if (is_support_disp_id(disp_id)) {
		if (disp_procfs.dsi_phy_rw_proc[disp_id]) {
			proc_remove(disp_procfs.dsi_phy_rw_proc[disp_id]);
			disp_procfs.dsi_phy_rw_proc[disp_id] = NULL;
		}
		ret = 0;
	} else {
		DISP_ERROR("unknown display id\n");
...
static int mi_disp_procfs_dsi_porch_rw_init(void *d_display, int disp_id)
{
	struct disp_display *dd_ptr = (struct disp_display *)d_display;
	struct disp_core *disp_core = mi_get_disp_core();
	int ret = 0;
...
static int mi_disp_procfs_dsi_porch_rw_deinit(void *d_display, int disp_id)
{
	struct disp_display *dd_ptr = (struct disp_display *)d_display;
	int ret = 0;
	...
	if (is_support_disp_id(disp_id)) {
		if (disp_procfs.dsi_porch_rw_proc[disp_id]) {
			proc_remove(disp_procfs.dsi_porch_rw_proc[disp_id]);
			disp_procfs.dsi_porch_rw_proc[disp_id] = NULL;
		}
		ret = 0;
	} else {
		DISP_ERROR("unknown display id\n");
...
static int mi_disp_procfs_pps_rw_init(void *d_display, int disp_id)
{
	struct disp_display *dd_ptr = (struct disp_display *)d_display;
	struct disp_core *disp_core = mi_get_disp_core();
	int ret = 0;
...
static int mi_disp_procfs_pps_rw_deinit(void *d_display, int disp_id)
{
	struct disp_display *dd_ptr = (struct disp_display *)d_display;
	int ret = 0;
	...
	if (is_support_disp_id(disp_id)) {
		if (disp_procfs.pps_rw_proc[disp_id]) {
			proc_remove(disp_procfs.pps_rw_proc[disp_id]);
			disp_procfs.pps_rw_proc[disp_id] = NULL;
		}
		ret = 0;
	} else {
		DISP_ERROR("unknown display id\n");
...
int mi_disp_procfs_init(void *d_display, int disp_id)
{
	...
	ret |= mi_disp_procfs_dsi_phy_rw_init(d_display, disp_id);
	ret |= mi_disp_procfs_dsi_porch_rw_init(d_display, disp_id);
	ret |= mi_disp_procfs_pps_rw_init(d_display, disp_id);
