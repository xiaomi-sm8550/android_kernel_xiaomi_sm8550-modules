...
static ssize_t dsi_phy_rw_store(struct device *device,
			   struct device_attribute *attr,
			   const char *buf, size_t count)
{
	struct disp_display *dd_ptr = to_disp_display(device);
	...
	if (dd_ptr->intf_type == MI_INTF_DSI) {
		mi_dsi_display_set_dsi_phy_rw(dd_ptr, (char *)buf);
	} else {
		DISP_ERROR("Unsupported display(%s intf)\n",
			get_disp_intf_type_name(dd_ptr->intf_type));
		ret = -EINVAL;
...
static ssize_t dsi_phy_rw_show(struct device *device,
			   struct device_attribute *attr,
			   char *buf)
{
	struct disp_display *dd_ptr = to_disp_display(device);
	...
	if (dd_ptr->intf_type == MI_INTF_DSI) {
		return mi_dsi_display_show_dsi_phy_rw(dd_ptr->display, buf, PAGE_SIZE);
	} else {
...
static ssize_t dsi_porch_rw_store(struct device *device,
			   struct device_attribute *attr,
			   const char *buf, size_t count)
{
	struct disp_display *dd_ptr = to_disp_display(device);
	...
	if (dd_ptr->intf_type == MI_INTF_DSI) {
		mi_dsi_display_set_dsi_porch_rw(dd_ptr, (char *)buf);
	} else {
		DISP_ERROR("Unsupported display(%s intf)\n",
			get_disp_intf_type_name(dd_ptr->intf_type));
		ret = -EINVAL;
...
static ssize_t dsi_porch_rw_show(struct device *device,
			   struct device_attribute *attr,
			   char *buf)
{
	struct disp_display *dd_ptr = to_disp_display(device);
	...
	if (dd_ptr->intf_type == MI_INTF_DSI) {
		return mi_dsi_display_show_dsi_porch_rw(dd_ptr->display, buf, PAGE_SIZE);
	} else {
		return snprintf(buf, PAGE_SIZE, "Unsupported display(%s intf)\n",
			get_disp_intf_type_name(dd_ptr->intf_type));
	}
...
static ssize_t pps_rw_store(struct device *device,
			   struct device_attribute *attr,
			   const char *buf, size_t count)
{
	return 0;
...
static ssize_t pps_rw_show(struct device *device,
			   struct device_attribute *attr,
			   char *buf)
{
	struct disp_display *dd_ptr = to_disp_display(device);
	...
	if (dd_ptr->intf_type == MI_INTF_DSI) {
		return mi_dsi_display_show_pps_rw(dd_ptr->display, buf, PAGE_SIZE);
	} else {
		return snprintf(buf, PAGE_SIZE, "Unsupported display(%s intf)\n",
			get_disp_intf_type_name(dd_ptr->intf_type));
	}
...
static DEVICE_ATTR_RW(dsi_phy_rw);
static DEVICE_ATTR_RW(dsi_porch_rw);
static DEVICE_ATTR_RW(pps_rw);
...
static struct attribute *disp_feature_attrs[] = {
	...
	&dev_attr_dsi_phy_rw.attr,
	&dev_attr_dsi_porch_rw.attr,
	&dev_attr_pps_rw.attr,
