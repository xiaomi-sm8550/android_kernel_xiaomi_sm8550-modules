...
{
	char msg[512] = "";
	const char* format = "{\"name\":\"audio_mbhc_abnormal\",\"audio_event\":{\"mbhc_stage\":\"%d\", \"mbhc_abnormal_reason\":\"%s\"},\"dgt\":\"null\",\"audio_ext\":\"null\" }";
	pr_debug("%s: stage is %d, reason is %s", __func__, stage, reason);
	snprintf(msg, sizeof(msg) - 1, format, stage, reason);
	...
	if (status == HEADSET_STATUS_RECORD_INDEX_PLUGOUT) {
		...
		switch(mask) {
			case SND_JACK_BTN_0:
				...
				headset_status[3] = headset_status[3] | (headset_key_order << HEADSET_EVENT_KEY_MEDIA_ORDER);
				} else {
			...
			case SND_JACK_BTN_1:
				...
				headset_status[1] = headset_status[1] | (headset_key_order << HEADSET_EVENT_KEY_PREVIOUS_ORDER);
				} else {
			...
			case SND_JACK_BTN_2:
				...
				headset_status[2] = headset_status[2] | (headset_key_order << HEADSET_EVENT_KEY_NEXT_ORDER);
				} else {
